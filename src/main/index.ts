...
async function readOcrSettings():Promise<StoredOcrSettings>{
  try{const raw=await fs.readFile(ocrSettingsPath(),'utf8');const value=JSON.parse(raw) as StoredOcrSettings;return{provider:value.provider==='remote'?'remote':'local',endpoint:value.endpoint??'',apiKey:value.apiKey?(safeStorage.isEncryptionAvailable()?safeStorage.decryptString(Buffer.from(value.apiKey,'base64')):value.apiKey):undefined}}
  catch{return{provider:'local',endpoint:''}}
}
async function writeOcrSettings(input:{provider:'local'|'remote';endpoint:string;apiKey?:string}):Promise<void>{
  const apiKey=input.apiKey?.trim();const stored:StoredOcrSettings={provider:input.provider,endpoint:input.endpoint.trim(),apiKey:apiKey?(safeStorage.isEncryptionAvailable()?safeStorage.encryptString(apiKey).toString('base64'):apiKey):undefined};await fs.mkdir(app.getPath('userData'),{recursive:true});await fs.writeFile(ocrSettingsPath(),JSON.stringify(stored,null,2),'utf8')
}
...
function pythonCandidates(){
  const packaged=join(process.resourcesPath,'app.asar.unpacked','python-runtime','Scripts','python.exe')
  const local=join(app.getAppPath(),'python-runtime','Scripts','python.exe')
  return [process.env.ZEMINLAB_PYTHON,packaged,local].filter((value):value is string=>Boolean(value))
}
async function runLocalPaddleOcr(dataUrl:string){
  ...
  const tempDir=await fs.mkdtemp(join(app.getPath('temp'),'zeminlab-ocr-'))
  const extension=match[1].toLowerCase()==='png'?'.png':'.jpg'
  const inputPath=join(tempDir,`input${extension}`),outputPath=join(tempDir,'result.json')
  await fs.writeFile(inputPath,Buffer.from(match[2],'base64'))
  const runner=join(app.getAppPath(),'tools','paddleocr_runner.py')
  if(!(await fs.stat(runner).catch(()=>null)))throw new Error('PaddleOCR çalıştırıcısı uygulama paketinde bulunamadı.')
  let lastError='Python bulunamadı.'
  for(const python of pythonCandidates().concat('python')){
    try{
      await execFileAsync(python,[runner,'--input',inputPath,'--output',outputPath],{windowsHide:true,maxBuffer:8*1024*1024,env:{...process.env,ZEMINLAB_OCR_DEVICE:process.env.ZEMINLAB_OCR_DEVICE??'cpu'}})
      const result=JSON.parse(await fs.readFile(outputPath,'utf8'))
      return result
    }catch(error){lastError=error instanceof Error?error.message:String(error)}
  }
  throw new Error(`Yerel PaddleOCR çalıştırılamadı: ${lastError}`)
}
...
app.whenReady().then(()=>{
 ...
 ipcMain.handle('project:save',async(_event,payload:unknown,currentPath?:string)=>{let filePath=currentPath;if(!filePath){const result=await dialog.showSaveDialog({title:'ZeminLab Projesini Kaydet',defaultPath:'Yeni Proje.zlproj',filters:projectSaveFilter});if(result.canceled||!result.filePath)return null;filePath=result.filePath.endsWith('.zlproj')?result.filePath:`${result.filePath}.zlproj`}const envelope={format:'ZeminLab',version:PROJECT_SCHEMA_VERSION,savedAt:new Date().toISOString(),data:payload};await fs.writeFile(filePath,JSON.stringify(envelope,null,2),'utf8');return filePath})
 ipcMain.handle('project:open',async()=>{const result=await dialog.showOpenDialog({title:'ZeminLab Projesini Aç',properties:['openFile'],filters:projectOpenFilter});if(result.canceled||!result.filePaths[0])return null;const filePath=result.filePaths[0];const raw=await fs.readFile(filePath,'utf8');const envelope=JSON.parse(raw) as {format?:string;version?:number;data?:unknown};if(envelope.format!=='ZeminLab'||typeof envelope.version!=='number'||envelope.data===undefined)throw new Error('Geçersiz veya desteklenmeyen ZeminLab proje dosyası.');if(envelope.version>PROJECT_SCHEMA_VERSION)throw new Error(`Bu proje dosyası daha yeni bir ZeminLab sürümüne ait (v${envelope.version}).`);return{filePath,data:envelope.data,version:envelope.version}})
 ...
 ipcMain.handle('ocr:analyze-image',async(_event,input:{dataUrl:string})=>{if(typeof input?.dataUrl!=='string')throw new Error('OCR görseli verilmedi.');return await runLocalPaddleOcr(input.dataUrl)})
 ...
 ipcMain.handle('report:export-pdf',async event=>{const window=BrowserWindow.fromWebContents(event.sender);if(!window)return null;const result=await dialog.showSaveDialog(window,{title:'Mühendislik Raporunu PDF Olarak Kaydet',defaultPath:'ZeminLab-Muhendislik-Raporu.pdf',filters:pdfFilter});if(result.canceled||!result.filePath)return null;const pdf=await window.webContents.printToPDF({landscape:false,pageSize:'A4',printBackground:true,displayHeaderFooter:false,margins:{top:0,bottom:0,left:0,right:0}});const filePath=result.filePath.endsWith('.pdf')?result.filePath:`${result.filePath}.pdf`;await fs.writeFile(filePath,pdf);return filePath})
