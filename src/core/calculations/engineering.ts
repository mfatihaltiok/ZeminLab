...
export function classifySoilISO14688(ll?: number, pi?: number) {
  ...
  const a = 0.73 * (ll - 20)
  const clay = pi >= a && pi >= 4
  const group = ll < 35 ? 'L' : ll <= 50 ? 'M' : 'H'
  return { code: clay ? `CI${group}` : `Si${group}`, description: clay ? 'Kil' : 'Silt', plasticity: group, aLine: a, isClay: clay }
}

export function stressAtDepth(depth: number, layers: Pick<SoilLayerInput, 'top' | 'bottom' | 'gamma' | 'gammaSat'>[], gwt: number) {
  return stressEngine(depth, layers, gwt)
}
...
export function sptCorrection(x: SptInput, sigmaVPrime: number) {
  const r = calculateSpt({
    nField: x.nField,
    energyRatio: x.energyRatio,
    boreholeDiameterMm: x.boreholeDiameter,
    sampler: x.sampler === 'without-liner' ? 'without-liner' : 'standard',
    effectiveStress: sigmaVPrime,
    fineContent: x.fines,
...
export function bearingCapacity(i: { B: number; L: number; Df: number; gamma: number; c: number; phi: number; FS: number; method: BearingMethod; waterReduction?: number }) {
  return { ...bearingEngine(i).value, method: i.method }
}
...
export const SOURCE_NOTES = {
  ...
  liquefaction: 'TBDY 2018 Bölüm 16.6: sıvılaşma değerlendirmesi. Uygulanan yöntem ve varsayımlar hesap izinde ayrıca gösterilir.',
  bearing: 'TBDY 2018 16.8.3.2 ve Denklem 16.8: yüzeysel temel taşıma gücü.',
  settlement: 'TBDY 2018 Bölüm 16: taşıma gücü ve yerdeğiştirme koşulları birlikte değerlendirilir; eksik zemin parametreleri korelasyon kaynağı ile belirtilmelidir.',
  foundation: 'TBDY 2018 16.7–16.8: temel tasarımı ve taban gerilmesi kontrolleri.',
