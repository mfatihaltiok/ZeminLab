...
export interface IdealizedSettlementResult {
  ...
  foundationEffectiveStress: number
  ready: boolean
  warnings: string[]
  source: string
}
...
function isCohesive(layer: IdealizedSoilLayer): boolean {
  ...
  return /(^|[^A-Z])(CI[LHM]|SI[LHM]|CL|CH|ML|MH)([^A-Z]|$)/.test(code)
    || code.includes('KIL') || code.includes('SILT') || code.includes('CLAY')
    || code.includes('ORGANIK') || code.includes('TURBA')
}
...
function effectiveStressAtDepth(layers: IdealizedSoilLayer[], depth: number, groundwaterDepth: number) {
  ...
  for (const layer of sorted) {
    ...
    const above = groundwaterDepth >= 0
      ? Math.max(0, Math.min(bottom, groundwaterDepth) - top)
      : bottom - top
...
export function calculateIdealizedSettlement(input: IdealizedSettlementInput): IdealizedSettlementResult {
  ...
  if (B <= 0 || L <= 0 || qGross <= 0) {
    return { method, layers: [], totalImmediate: 0, totalConsolidation: 0, totalSettlement: 0, influenceDepth: 0, netFoundationPressure: 0, foundationEffectiveStress: 0, ready: false, warnings: [...warnings, 'Temel genişliği, uzunluğu ve temel yükü pozitif olmalıdır.'], source: 'ZeminLab İdealize Zemin Profili oturma motoru' }
  }
  ...
  for (const layer of layers) {
    ...
    if (cohesive) {
      ...
      if (!finite(layer.compressionIndexCc) || !finite(layer.initialVoidRatio) || midStress.effective <= 0) {
        status = 'VERİ EKSİK'
        note = 'Konsolidasyon için Cc ve başlangıç boşluk oranı e₀ gerekir.'
      }
      ...
      if (finite(E) && E > 0) {
        ...
        immediate = Math.max(0, (deltaSigma / E) * thickness * (1 - nu * nu))
        note = `E=${E.toFixed(1)} kPa, ν=${nu.toFixed(2)}`
      } else {
        status = 'VERİ EKSİK'
        note = 'Elastik ani oturma için Eoed/E ve tercihen ν gerekir.'
      }
    }
    ...
    results.push({
      layerId: layer.id, order: layer.order, soilName: layer.soilName, soilCode: layer.soilCode,
      topDepth: top, bottomDepth: bottom, thickness, midDepth,
      sigmaV0: midStress.total, porePressure: midStress.porePressure,
      sigmaV0Effective: midStress.effective, deltaSigma,
      sigmaVFinal: midStress.total + deltaSigma, sigmaVFinalEffective: finalEffective,
      representativeN60: layer.representativeN60 ?? layer.representativeSptN,
      Es: layer.constrainedModulus ?? layer.oedometricModulus, poissonRatio: layer.poissonRatio,
      immediateSettlement: immediate, consolidationSettlement: consolidation,
      totalSettlement: immediate + consolidation, method: methodName, status, note
    })
  }
  ...
  return {
    method, layers: results, totalImmediate, totalConsolidation, totalSettlement: totalImmediate + totalConsolidation,
    influenceDepth, netFoundationPressure: qNet, foundationEffectiveStress: baseStress.effective,
    ready: results.length > 0 && !results.some(x => x.status === 'VERİ EKSİK'), warnings,
    source: 'Eski Zemin Etüdü/Jet Grout yazılımındaki Burland & Burbidge + tabaka bazlı gerilme yaklaşımı temel alınarak yeniden düzenlendi; killerde Terzaghi 1B konsolidasyon hesabı kullanılır.'
  }
}
