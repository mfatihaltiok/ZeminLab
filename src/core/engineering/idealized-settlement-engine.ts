...
function consolidationSettlement(layer:IdealizedSoilLayer,thickness:number,sigma0:number,sigma1:number){if(!finite(layer.compressionIndexCc)||!finite(layer.initialVoidRatio)||sigma0<=0||sigma1<=sigma0)return{value:0,ok:false};const Cc=layer.compressionIndexCc!,e0=layer.initialVoidRatio!,hasCr=finite(layer.recompressionIndexCr)&&layer.recompressionIndexCr!>=0,Cr=hasCr?layer.recompressionIndexCr!:Cc,pc=finite(layer.preconsolidationPressure)&&layer.preconsolidationPressure!>sigma0?layer.preconsolidationPressure!:sigma0;if(pc>sigma0&&!hasCr)return{value:0,ok:false};let strain=0;if(sigma1<=pc)strain=Cr*Math.log10(sigma1/sigma0);else{strain=Cr*Math.log10(Math.max(pc/sigma0,1));strain+=Cc*Math.log10(Math.max(sigma1/pc,1))}return{value:Math.max(0,thickness*strain/(1+e0)*1000),ok:true}}
export function calculateIdealizedSettlement(input:IdealizedSettlementInput):IdealizedSettlementResult{
  ...
  if(method==='schmertmann'&&(!finite(input.timeYears)||input.timeYears!<0.1))warnings.push('Schmertmann hesabı için t≥0.1 yıl zaman parametresi girilmeden sonuç hazır kabul edilmez.')
