...
function rdAtDepth(z:number){const depth=Math.max(z,0);if(depth<=9.15)return 1-0.00765*depth;if(depth<=23)return 1.174-0.0267*depth;if(depth<=30)return 0.744-0.008*depth;return .50}
...
export function liquefactionProfile(input:LiquefactionProfileInput):LiquefactionProfileResult{
  ...
  const rows=[...input.spt].sort((a,b)=>a.depth-b.depth).map((record):LiquefactionProfileRow=>{
    ...
    const spt=calculateSpt({nField:record.nField,energyRatio:record.energyRatio,boreholeDiameterMm:record.boreholeDiameterMm,sampler:record.sampler,samplerCorrection:record.samplerCorrection,rodLengthM:record.rodLengthM,effectiveStress:stress.sigmaVPrime,fineContent:record.fineContent,applyOverburden:true,applyDilatancy:input.applyDilatancy??false})
    const fines=Math.max(0,record.fineContent??0),{alpha,beta}=fineContentCorrection(fines),n1_60f=alpha+beta*spt.n1_60,rd=rdAtDepth(record.depth)
    const soil=input.layers.find(l=>record.depth>=l.top&&record.depth<l.bottom)?.soil
    const trace:SptTraceStep[]=[...spt.trace,{symbol:'α',title:'İnce dane katsayısı',formula:'Ek 16B.2.2',value:alpha,note:`IDI = ${fines.toFixed(2)} %`},{symbol:'β',title:'İnce dane katsayısı',formula:'Ek 16B.2.2',value:beta},{symbol:'(N₁)₆₀f',title:'İnce dane düzeltilmiş SPT',formula:'(N₁)₆₀f = α + β(N₁)₆₀',value:n1_60f}]
    ...
    if(stress.covered<Math.max(0,record.depth)-1e-9){trace.push({symbol:'Kapsama',title:'Katman kapsamı',formula:'ΣΔz = z',value:stress.covered,unit:'m',note:'SPT derinliğinin bir bölümü katman tanımı dışında kaldı; gerilme hesabı eksik veri içeriyor.'});return{...base,liquefactionCheck:'not-evaluable',trace}}
    if(record.depth<=input.gwt+1e-9){trace.push({symbol:'YASS',title:'Doygunluk kontrolü',formula:'z > YASS',value:input.gwt,unit:'m',note:'SPT noktası YASS altında olmadığı için sıvılaşma hesabı uygulanmadı.'});return{...base,liquefactionCheck:'not-evaluable',trace}}
    if(spt.n1_60>=30||n1_60f>=30){trace.push({symbol:'(N₁)₆₀',title:'Tetikleme sınırı',formula:'(N₁)₆₀ < 30',value:spt.n1_60,note:'TBDY 2018 16.6.5 kapsamında SPT tabanlı sıvılaşma değerlendirmesi için eşik sağlanmadı.'});return{...base,liquefactionCheck:'not-evaluable',trace}}
    const crrRaw=1/(34-n1_60f)+n1_60f/135+50/Math.pow(10*n1_60f+45,2)-1/200,crrM75=Math.min(2,Math.max(0,crrRaw)),tauResistance=crrM75*CM*stress.sigmaVPrime,tauEarthquake=.65*(.4*input.Sds)*stress.sigmaV*rd,FS=tauResistance/Math.max(tauEarthquake,1e-9)
    trace.push({symbol:'CRR₇.₅',title:'Çevrimsel dayanım oranı',formula:'Ek 16B.3.2',value:crrM75,note:crrRaw>2?'CRR değeri 2.0 üst sınırına sınırlandırıldı.':undefined},{symbol:'Cᴹ',title:'Deprem büyüklüğü düzeltmesi',formula:'Cᴹ = 10²·²⁴ / Mʷ²·⁵⁶',value:CM},{symbol:'τᴿ',title:'Sıvılaşma direnci',formula:'τᴿ = CRR₇.₅·Cᴹ·σ′ᵥ₀',value:tauResistance,unit:'kPa'},{symbol:'rᵈ',title:'Gerilme azaltma katsayısı',formula:'Ek 16B.4.1, parçalı bağıntı',value:rd},{symbol:'τdeprem',title:'Deprem kayma gerilmesi',formula:'τdeprem = 0.65·(0.4SDS)·σᵥ₀·rᵈ',value:tauEarthquake,unit:'kPa'},{symbol:'FS',title:'Sıvılaşmaya karşı güvenlik',formula:'FS = τᴿ/τdeprem',value:FS,note:'TBDY 2018 güvenlik koşulu: FS ≥ 1.10.'})
