...
export function liquefactionProfile(input:LiquefactionProfileInput):LiquefactionProfileResult{
  ...
  const rows=input.spt.filter(x=>finite(x.depth)&&x.depth>=0).sort((a,b)=>a.depth-b.depth).map((record):LiquefactionProfileRow=>{
    ...
    const fines=clamp(fineContent??0,0,100),fc=finesCorrection(fines),n1_60f=fc.alpha+fc.beta*npt.n1_60
    ...
    if(zfBlocked){trace.push({symbol:'ZF',title:'Saha özel zemin davranış analizi',formula:'TBDY 16.5.1.3',value:0,note:'ZF saha özel analiz tamamlanmadı.'});return{...base,status:'VERİ EKSİK',conclusion:'VERİ EKSİK',liquefactionCheck:'not-evaluable',trace}}
    if(!npt.correctionReady||!npt.normalizationReady){trace.push({symbol:'SPT',title:'SPT düzeltme/veri hazırlığı',formula:'CE·CB·CS·CR + CN',value:0,note:'N60 ve/veya (N1)60 için gerekli girdiler tamamlanmadı.'});return{...base,status:'VERİ EKSİK',conclusion:'VERİ EKSİK',liquefactionCheck:'not-evaluable',trace}}
    ...
    const crrM75Value=crrM75(n1_60f)
    if(crrM75Value===undefined){trace.push({symbol:'CRR7.5',title:'CRR uygulanabilirlik sınırı',formula:'(N1)60f<34',value:n1_60f,note:'Ek 16B bağıntısının uygulanabilir aralığı dışında.'});return{...base,status:'TETİKLENME DEĞERLENDİRMESİ',conclusion:'DEĞERLENDİRİLMEDİ',liquefactionCheck:'not-evaluable',trace}}
    const crrM75=crrM75Value
