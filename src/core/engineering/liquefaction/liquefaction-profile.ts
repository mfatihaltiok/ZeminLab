...
function layerAt(depth:number,layers:SoilLayerStressInput[]){return [...layers].sort((a,b)=>a.top-b.top).find(l=>depth>=l.top&&depth<l.bottom)??layers[layers.length-1]}
function stress(depth:number,record:LiquefactionSptRecord,layers:SoilLayerStressInput[],gwt:number){const layer=layerAt(depth,layers);const gammaT=record.unitWeightTPerM3!=null&&record.unitWeightTPerM3>0?record.unitWeightTPerM3:(layer?.gamma!=null&&layer.gamma>0?layer.gamma/10:1.7);const gammaK=gammaT*10;const sigmaV=depth*gammaK;const u=depth>gwt?9.81*(depth-gwt):0;return{sigmaV,sigmaVPrime:Math.max(.1,sigmaV-u),u,gammaK}}
...
export function liquefactionProfile(i:LiquefactionProfileInput):LiquefactionProfileResult{
 ...
 const rows=[...i.spt].filter(x=>x.testType!=='UD'&&Number.isFinite(x.depth)&&x.depth>0&&Number.isFinite(x.nField)&&x.nField>=0).sort((a,b)=>a.depth-b.depth).map(record=>{
  const layer=layerAt(record.depth,i.layers);const soilType=record.soilType||'';const cohesive=isCohesiveSoil(soilType);const st=stress(record.depth,record,i.layers,i.groundwaterDepth);const f=correctionFactors(record);
  ...
  const n60=record.nField*f.ce*f.cb*f.cs*f.cr;const n1_60=n60*cn;
  const dilatancyApplied=belowGwt&&!cohesive&&n1_60>15;const n1_60_dilatancy=dilatancyApplied?15+.5*(n1_60-15):n1_60;
  const fines=record.fines??record.fineContent??layer?.finesContent??0;const fine=fineCorrection(n1_60_dilatancy,fines);const rr=rd(record.depth);
  const csr=.65*(.4*i.Sds)*(st.sigmaV/Math.max(.1,st.sigmaVPrime))*rr;
  const crr=crrM75(fine.n1_60f);const tauEarthquake=.65*(.4*i.Sds)*st.sigmaV*rr;const tauResistance=crr*CM*st.sigmaVPrime;const fsL=tauEarthquake>0?tauResistance/tauEarthquake:9.9;
  ...
  const pi=record.plasticityIndex??layer?.plasticityIndex??(cohesive?12:0);const codeUpper=soilType.toUpperCase().replace(/İ/g,'I');const isSandySoil=['SA','SISA','CLSA','GRSA'].includes(codeUpper)||codeUpper.includes('KUM')||!cohesive;
  ...
  else if(isSandySoil&&fines>35&&n1_60>20)explanation=`IDI > 35% ve (N1)60 > 20.`
  else explanation=`Rτ/τdeprem = ${fsL.toFixed(3)} ≥ 1.10.`;
  let isLiquefiable:LiquefactionProfileRow['isLiquefiable']='SIVILAŞMA YOK';if(conclusion==='SIVILAŞMA VAR'||(cohesive&&claySofteningRisk==='YÜKSEK'))isLiquefiable='YÜKSEK';else if(fsL<1.3)isLiquefiable='ORTA';
 ...
 return{rows,method:'TBDY 2018 Ek 16B · SPT → CN/CR/CS/CB/CE → N1,60 → IDI düzeltmesi → CRR_M7.5 → CM → Rτ → τdeprem → Rτ/τdeprem',source:'TBDY 2018, Bölüm 16.6 ve Ek 16B.2-Ek 16B.4; resmi AFAD yönetmelik metni.'}
}
