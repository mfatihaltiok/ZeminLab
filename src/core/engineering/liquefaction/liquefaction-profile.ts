...
export function liquefactionProfile(input:LiquefactionProfileInput):LiquefactionProfileResult{
 ...
 const rows=input.spt.filter(x=>finite(x.depth)&&x.depth>=0).sort((a,b)=>a.depth-b.depth).map((record):LiquefactionProfileRow=>{
  ...
  if(n1f>=34)return{...base,alpha:fc.alpha,beta:fc.beta,n1_60f:n1f,status:'TETİKLENME DEĞERLENDİRMESİ',conclusion:'DEĞERLENDİRİLMEDİ',liquefactionCheck:'not-evaluable',triggerRequired:true,postLiquefactionRequired:true,trace:[...trace,{symbol:'(N1)60f',title:'CRR bağıntı sınırı',formula:'(N1)60f<34',value:n1f,note:'CRR bağıntısı uygulanmadı.'}]}
  ...
  const result={...base,alpha:fc.alpha,beta:fc.beta,n1_60f:n1f,crrM75,CM,tauResistance,tauEarthquake,FS,triggerRequired:true,postLiquefactionRequired:true,status:'TETİKLENME DEĞERLENDİRMESİ' as const,conclusion:FS<1.10?'SIVILAŞMA RİSKİ VAR' as const:'SIVILAŞMA RİSKİ YOK' as const,liquefactionCheck:'evaluate' as const,trace:[...trace,
