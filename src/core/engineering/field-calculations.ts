...
export function deriveSptValues(borehole:BoreholeRecord,record:SptRecord,laboratories:LaboratoryRecord[] = []):SptDerivedValues { const nField=fieldN(record); if(nField===undefined) return {nField,ce:1,cb:1,cs:1,cr:1,cn:1,n60:0,n1_60:0,dilatancyApplied:false,trace:[],overburdenCorrection:1,overburdenCorrectionApplied:false}; const stress=stressAtDepth(borehole,record.depth,laboratories); const cfg=record.correction??{}; const lab=linkedLabForSpt(laboratories,borehole.id,record.id,record.depth); const fineContent=cfg.fineContent??lab?.finesContent??lab?.sieve200Passing??layerAtDepth(borehole,record.depth)?.finesContent; const result=calculateSpt({nField,energyRatio:cfg.energyRatio,effectiveStress:stress.effectiveStress,fineContent,applyOverburden:cfg.applyOverburdenCorrection??true,applyDilatancy:cfg.applyDilatancyCorrection??false}); return {...result,verticalStress:stress.verticalStress,effectiveStress:stress.effectiveStress,stressSource:stress.source,overburdenCorrection:result.cn,overburdenCorrectionApplied:stress.effectiveStress!=null,n60DilatancyCorrected:result.n1_60_dilatancy} }
