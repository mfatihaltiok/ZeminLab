...
export interface SptDerivedValues extends SptEngineResult { verticalStress?: number; effectiveStress?: number }
...
function stressAtDepth(borehole: BoreholeRecord, depth: number) {
  const layer = layerAtDepth(borehole, depth); if (!layer?.unitWeight) return { verticalStress: undefined, effectiveStress: undefined }
  const gamma = layer.unitWeight; const gammaSat = layer.saturatedUnitWeight ?? gamma; const gwt=borehole.groundwaterDepth; const z=Math.max(0,depth); const above=gwt===undefined?z:Math.min(z,Math.max(0,gwt)); const below=z-above; const verticalStress=above*gamma+below*gammaSat; const porePressure=gwt!==undefined&&z>gwt?(z-gwt)*9.80665:0; return { verticalStress, effectiveStress: Math.max(0,verticalStress-porePressure) }
}

export function deriveSptValues(borehole: BoreholeRecord, record: SptRecord): SptDerivedValues {
  const nField=fieldN(record); if (nField===undefined) return { ...calculateSpt({ nField: 0, applyOverburden: false }), nField: undefined as never }
  const stress=stressAtDepth(borehole,record.depth); const cfg=record.correction ?? {}
  const result=calculateSpt({ nField, energyRatio: cfg.energyRatio, boreholeDiameterMm: undefined, sampler: undefined, rodLengthM: undefined, effectiveStress: stress.effectiveStress, fineContent: cfg.fineContent ?? borehole.lithology.find((l)=>record.depth>=l.from&&record.depth<=l.to)?.finesContent, applyOverburden: cfg.applyOverburdenCorrection ?? true, applyDilatancy: cfg.applyDilatancyCorrection ?? false })
  return { ...result, verticalStress: stress.verticalStress, effectiveStress: stress.effectiveStress }
}
