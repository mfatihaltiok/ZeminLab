...
export interface SurfaceFoundationInput{B:number;L:number;Df:number;gamma1:number;gamma2:number;c:number;phi:number;verticalLoad:number;horizontalLoad?:number;momentX?:number;momentY?:number;groundSlope?:number;baseSlope?:number;resistanceFactor?:number;method?:SurfaceFoundationMethod;safetyFactor?:number;undrainedCu?:number;foundationType?:FoundationType;unitSystem?:UnitSystem}
export interface SurfaceFoundationResult{Nq:number;Nc:number;Ngamma:number;sc:number;sq:number;sg:number;dc:number;dq:number;dg:number;ic:number;iq:number;ig:number;gc:number;gq:number;gg:number;bc:number;bq:number;bg:number;surcharge:number;ex:number;ey:number;Be:number;Le:number;effectiveArea:number;qk:number;qt:number;qo:number;utilization:number;adequate:boolean;ultimateClassical?:number;allowableClassical?:number;undrainedQk?:number;method:SurfaceFoundationMethod;foundationType:FoundationType;warnings:string[]}

const rad=(deg:number)=>deg*Math.PI/180
const clamp=(x:number,min:number,max:number)=>Math.max(min,Math.min(max,x))

/**
 * ZeminLab giriş birimleri proje birim sistemindedir: m, tonf/tonf·m/tonf·m² veya kN/kN·m/kN·m².
 * Bu fonksiyon aynı kuvvet sistemi içinde boyutsal olarak çalışır; tonf ve kN arasında çevrim gerektiren
 * sabitler burada kullanılmaz. Böylece seçilen proje birimi sonuç birimine doğrudan taşınır.
 */
export function calculateSurfaceFoundation(i:SurfaceFoundationInput):SurfaceFoundationResult{
  ...
  if(i.verticalLoad<=0)throw new Error('Düşey temel yükü proje kuvvet biriminde pozitif olmalıdır.')
  ...
  if(method==='Vesic'||method==='TBDY-2018') Ngamma=phi===0?0:2*(Nq+1)*t

  const N=i.verticalLoad,H=Math.abs(i.horizontalLoad??0)
  const ex=(i.momentY??0)/N,ey=(i.momentX??0)/N
  const Be=i.B-2*Math.abs(ex),Le=i.L-2*Math.abs(ey)
  const effectiveArea=Math.max(0,Be)*Math.max(0,Le),warnings:string[]=[]
  ...
  const Bp=Math.max(Math.min(Math.max(Be,1e-9),Math.max(Le,1e-9)),1e-9)
  const Lp=Math.max(Math.max(Be,1e-9),Math.max(Le,1e-9)),ratio=Bp/Lp
  ...
  let ic=1,iq=1,ig=1
  if(method!=='Terzaghi'){
    const loadRatio=Math.min(0.999999,H/Math.max(N,1e-9)),m=(2+ratio)/(1+ratio),common=Math.max(0,1-loadRatio)
    iq=common**m;ig=common**(m+1)
  ...
  if(method==='TBDY-2018'&&foundationType==='radye')warnings.push('Radye temel TBDY 16.8 kapsamındaki yüzeysel temel kontrolüdür; geniş temel davranışı ve oturma ayrıca değerlendirilmelidir.')
