...
export interface JetGroutProjectParameters{unitSystem?:UnitSystem;columnDiameter?:number;spacing?:number;layout?:'square'|'triangular';qSoil?:number;qColumn?:number;cSoil?:number;cColumn?:number;EsSoil?:number;EsColumn?:number;foundationThickness?:number;columnFrictionAngle?:number;interfaceCohesion?:number;interfaceFrictionAngle?:number;interfaceNormalStress?:number;soilPoissonRatio?:number}
...
export interface SoilParameters{unitWeight:number;saturatedUnitWeight:number;cohesion:number;frictionAngle:number;undrainedCohesion?:number;groundwaterDepth?:number;surfaceSlope:number;foundationBaseSlope:number;finesContent:number;liquefactionContinuousOrThickLens?:boolean;classification:SoilClassification}
export interface FoundationParameters{foundationType:FoundationType;footingWidth:number;footingLength:number;footingDepth:number;safetyFactor:number;verticalLoad:number;horizontalLoad:number;momentX:number;momentY:number;resistanceFactorRv:number;vtX:number;vtY:number;structuralWeight:number;baseFrictionTanDelta?:number;passiveResistanceCharacteristic:number;usePassiveResistance:boolean}
export interface ProjectInfo{id:string;title:string;projectNo:string;date:string;location:string;province:string;district:string;address:string;parcelInfo:string;pafta:string;ada:string;parsel:string;zoningStatus:string;engineer:string;clientName:string;firmName:string;buildingType:string;basementCount:number;normalFloorCount:number;unitSystem:UnitSystem;geophysical:GeophysicalParameters;seismic:SeismicParameters;soilParameters:SoilParameters;foundationParameters:FoundationParameters;jetGrout:JetGroutProjectParameters;visualDocuments:ProjectVisualDocuments}
export const defaultProjectInfo:ProjectInfo={id:'',title:'',projectNo:'',date:'',location:'',province:'',district:'',address:'',parcelInfo:'',pafta:'',ada:'',parsel:'',zoningStatus:'',engineer:'',clientName:'',firmName:'',buildingType:'',basementCount:0,normalFloorCount:0,unitSystem:'ton-m',geophysical:{soilGroupSource:'VS30',siteSpecificResponseAnalysisCompleted:false},seismic:{},soilParameters:{unitWeight:0,saturatedUnitWeight:0,cohesion:0,frictionAngle:0,undrainedCohesion:undefined,groundwaterDepth:undefined,surfaceSlope:0,foundationBaseSlope:0,finesContent:0,liquefactionContinuousOrThickLens:undefined,classification:{system:'TS EN ISO 14688-2'}},foundationParameters:{foundationType:'tekil',footingWidth:0,footingLength:0,footingDepth:0,safetyFactor:3,verticalLoad:0,horizontalLoad:0,momentX:0,momentY:0,resistanceFactorRv:ENGINEERING_CONSTANTS.TBDY_GAMMA_RV,vtX:0,vtY:0,structuralWeight:0,baseFrictionTanDelta:undefined,passiveResistanceCharacteristic:0,usePassiveResistance:false},jetGrout:{layout:'square',unitSystem:'kN-m'},visualDocuments:{}}
export function classifyVs30(vs30?:number):GeophysicalParameters['soilGroup']{if(vs30===undefined||!Number.isFinite(vs30)||vs30<=0)return undefined;if(vs30>=1500)return'ZA';if(vs30>=760)return'ZB';if(vs30>=360)return'ZC';if(vs30>=180)return'ZD';return'ZE'}
