...
export interface JetGroutProjectParameters{
  columnDiameter?:number;spacing?:number;layout?:'square'|'triangular';qSoil?:number;qColumn?:number;cSoil?:number;cColumn?:number;EsSoil?:number;EsColumn?:number
  foundationThickness?:number;columnFrictionAngle?:number;interfaceCohesion?:number;interfaceFrictionAngle?:number;soilPoissonRatio?:number
}
...
export interface SoilParameters{
  unitWeight:number;saturatedUnitWeight:number;cohesion:number;frictionAngle:number;undrainedCohesion?:number;groundwaterDepth?:number
  surfaceSlope:number;foundationBaseSlope:number;finesContent:number;liquefactionContinuousOrThickLens?:boolean;classification:SoilClassification
}
export interface FoundationParameters{
  foundationType:FoundationType;footingWidth:number;footingLength:number;footingDepth:number;safetyFactor:number
  structuralWeight:number;verticalLoad:number;horizontalLoad:number;momentX:number;momentY:number
  resistanceFactorRv?:number;vtX:number;vtY:number;baseFrictionTanDelta?:number;passiveResistanceCharacteristic:number;usePassiveResistance:boolean
}
...
export const TBDY_GAMMA_RV=1.4
export const TBDY_GAMMA_RH=1.1
export const TBDY_GAMMA_RP=1.4
...
export const defaultProjectInfo:ProjectInfo={
  ...
  foundationParameters:{foundationType:'tekil',footingWidth:0,footingLength:0,footingDepth:0,safetyFactor:0,verticalLoad:0,horizontalLoad:0,momentX:0,momentY:0,resistanceFactorRv:undefined,vtX:0,vtY:0,baseFrictionTanDelta:undefined,passiveResistanceCharacteristic:0,usePassiveResistance:false},
...
export function classifyVs30(vs30?:number):GeophysicalParameters['soilGroup']{
  ...
  if(vs30>=1500)return'ZA';if(vs30>=760)return'ZB';if(vs30>=360)return'ZC';if(vs30>=180)return'ZD';return'ZE'
}
export function calculateSdsSeismic(ss?:number,fs?:number):number|undefined{return !Number.isFinite(ss)||!Number.isFinite(fs)||ss!<0||fs!<=0?undefined:ss!*fs!}
export function calculateSd1Seismic(s1?:number,f1?:number):number|undefined{return !Number.isFinite(s1)||!Number.isFinite(f1)||s1!<0||f1!<=0?undefined:s1!*f1!}
export function determineDts(sds:number|undefined,bks:BuildingUseClass|undefined):EarthquakeDesignClass|undefined{
  ...
  if(sds!<.33)return bks===1?'4a':'4';if(sds!<.5)return bks===1?'3a':'3';if(sds!<.75)return bks===1?'2a':'2';return bks===1?'1a':'1'
}
export function normalizeProjectInfo(value:Partial<ProjectInfo>):ProjectInfo{
  ...
  const horizontalLoad=Math.hypot(Number.isFinite(foundation.vtX)?foundation.vtX:0,Number.isFinite(foundation.vtY)?foundation.vtY:0)
  const sds=calculateSdsSeismic(seismic.ss,seismic.fs)??(Number.isFinite(seismic.sds)?seismic.sds:undefined)
  const sd1=calculateSd1Seismic(seismic.s1,seismic.f1)??(Number.isFinite(seismic.sd1)?seismic.sd1:undefined)
  const dts=determineDts(sds,seismic.bks)
  return {...defaultProjectInfo,...value,unitSystem,geophysical:{...defaultProjectInfo.geophysical,...(value.geophysical??{})},
    seismic:{...seismic,sds,sd1,dts},soilParameters:{...soil,classification:{...defaultProjectInfo.soilParameters.classification,...(soil.classification??{})}},
    foundationParameters:{...foundation,foundationType,structuralWeight,verticalLoad:structuralWeight,horizontalLoad},
    jetGrout:{...defaultProjectInfo.jetGrout,...(value.jetGrout??{})},visualDocuments:value.visualDocuments??{}}
}
