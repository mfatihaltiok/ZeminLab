...
function App(){
 const [screen,setScreen]=useState<ScreenId>('dashboard');const project=useProjectInfo();const [boreholes,setBoreholes]=useState<BoreholeRecord[]>([]);const [labs,setLabs]=useState<LaboratoryRecord[]>([]);const [selectedBoreholeId,setSelectedBoreholeId]=useState('');const [idealizedSoilProfile,setIdealizedSoilProfile]=useState<IdealizedSoilProfile>();const [projectPath,setProjectPath]=useState<string>();const selectedBorehole=boreholes.find(b=>b.id===selectedBoreholeId)
 const saveProject=async()=>{try{const path=await window.api.project.save({projectInfo:project,boreholes,labs,idealizedSoilProfile},projectPath);if(path)setProjectPath(path)}catch(e){console.error('FALUZMN proje kaydı başarısız:',e)}}
 const openProject=async()=>{try{const r=await window.api.project.open();if(!r)return;const data=migrateProjectData(r.data,r.version);updateProjectInfo(data.projectInfo);setBoreholes(data.boreholes);setLabs(data.labs);setSelectedBoreholeId(data.boreholes[0]?.id??'');setIdealizedSoilProfile(data.idealizedSoilProfile);setProjectPath(r.filePath);setScreen('dashboard')}catch(e){console.error('FALUZMN proje açma başarısız:',e)}}
 ...
 const importSptOcr=(rows:SptCandidate[])=>{if(!selectedBorehole)return;const existingDepths=new Set(selectedBorehole.spt.map(row=>Number(row.depth.toFixed(3))));const additions:SptRecord[]=rows.filter(row=>!existingDepths.has(Number(row.depth.toFixed(3)))).map((row,index)=>({id:`SPT-OCR-${crypto.randomUUID()}-${index}`,depth:row.depth,depthTo:row.depth+0.45,testType:'SPT',n1:row.n1,n2:row.n2,n3:row.n3,source:'ocr',confirmed:true,provenance:{...ocrProvenance('PaddleOCR + Docling'),approved:true,note:'Kullanıcı OCR önizlemesini onayladı.'}}));if(!additions.length)return;setBoreholes(current=>current.map(b=>b.id===selectedBorehole.id?{...b,spt:[...b.spt,...additions].sort((a,b)=>a.depth-b.depth)}:b))}
 const importLabOcr=(targetId:string,rows:LabCandidate[])=>{const allowedFields=new Set<keyof LaboratoryRecord>(['waterContent','sieve10Passing','sieve200Passing','liquidLimit','plasticLimit','plasticityIndex','unitWeight','uuC','uuPhi','consolidationCc','consolidationCs','elasticModulus','poissonRatio','directShearC','directShearPhi','density','porosity','voidRatio','c','phi','finesContent']);setLabs(current=>current.map(lab=>{if(lab.id!==targetId)return lab;const patch:Partial<LaboratoryRecord>={};for(const row of rows){const key=row.field as keyof LaboratoryRecord;if(allowedFields.has(key))(patch as any)[key]=row.value}const next=applyLaboratoryDerivedValues({...lab,...patch,source:'ocr',confirmed:true,provenance:{...ocrProvenance('PaddleOCR + Docling'),approved:true,note:'Kullanıcı OCR önizlemesini onayladı.'}});return next}))}
 const field=<><FieldCommandBar boreholes={boreholes} labs={labs} selectedBoreholeId={selectedBoreholeId} onSelectedBoreholeChange={setSelectedBoreholeId} onBoreholesChange={setBoreholes} onLabsChange={setLabs}/><EngineeringOcrImport mode="spt" onSptImport={importSptOcr}/><EngineeringOcrImport mode="laboratory" laboratoryTargets={labs.filter(l=>l.boreholeId===selectedBoreholeId).map(l=>({id:l.id,sampleId:l.sampleId,depth:l.depth}))} onLaboratoryImport={importLabOcr}/><FieldInvestigation boreholes={boreholes} labs={labs} selectedBoreholeId={selectedBoreholeId} onSelectedBoreholeChange={setSelectedBoreholeId} onBoreholesChange={setBoreholes} onLabsChange={setLabs}/></>
