...
export default function App(){const [screen,setScreen]=useState<ScreenId>('dashboard');const project=useProjectInfo();const [boreholes,setBoreholes]=useState<BoreholeRecord[]>([]);const [labs,setLabs]=useState<LaboratoryRecord[]>([]);const [projectPath,setProjectPath]=useState<string>();const saveProject=async()=>{try{const path=await window.api.project.save({projectInfo:project,boreholes,labs},projectPath);if(path)setProjectPath(path)}catch(error){console.error('ZeminLab proje kaydı başarısız:',error)}};const openProject=async()=>{try{const result=await window.api.project.open();if(!result||!isProjectDocument(result.data))return;updateProjectInfo(normalizeProjectInfo(result.data.projectInfo));setBoreholes(result.data.boreholes);setLabs(result.data.labs);setProjectPath(result.filePath);setScreen('dashboard')}catch(error){console.error('ZeminLab proje açma başarısız:',error)}};const newProject=()=>{updateProjectInfo({...defaultProjectInfo,id:crypto.randomUUID(),date:new Date().toISOString().slice(0,10)});setProjectPath(undefined);setBoreholes([]);setLabs([]);setScreen('dashboard')};const fieldWorkspace=<><DocumentOcrPanel boreholes={boreholes} labs={labs} onBoreholesChange={setBoreholes} onLabsChange={setLabs}/><FieldInvestigation boreholes={boreholes} labs={labs} onBoreholesChange={setBoreholes} onLabsChange={setLabs}/></>;const foundationWorkspace=<><Foundation/><Foundation3DView boreholes={boreholes}/></>;const content:Record<ScreenId,ReactNode>={dashboard:<Dashboard onNavigate={setScreen}/>, 'project-info':<ProjectInfoScreenV2/>, field:fieldWorkspace, 'borehole-log':<BoreholeLogScreen boreholes={boreholes} labs={labs} onBoreholesChange={setBoreholes}/>, profile:<SoilProfileScreen boreholes={boreholes} labs={labs}/>, 'bearing-capacity':<BearingCapacityScreen/>, settlement:<Settlement boreholes={boreholes}/>, liquefaction:<Liquefaction boreholes={boreholes} labs={labs}/>, foundation:foundationWorkspace, 'jet-grout':<JetGrout/>, report:<EngineeringReportScreen boreholes={boreholes} labs={labs}/>, 'unit-converter':<UnitConverterScreen/>};return <WorkspaceShell screen={screen} onScreenChange={setScreen} onNewProject={newProject} onOpenProject={openProject} onSaveProject={saveProject}>{content[screen]}</WorkspaceShell>}
