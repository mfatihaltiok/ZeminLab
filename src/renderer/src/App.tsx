...
import './assets/engineering-render.css'
...
function App(){
 ...
 useEffect(()=>{void window.api.fieldCache.load().then(cached=>{if(!cached)return;if(Array.isArray(cached.boreholes))setBoreholes(cached.boreholes as BoreholeRecord[]);if(Array.isArray(cached.laboratories))setLabs(cached.laboratories as LaboratoryRecord[]);if(Array.isArray(cached.boreholes)&&cached.boreholes.length)setSelectedBoreholeId((cached.boreholes as BoreholeRecord[])[0]?.id??'');setStatusText(cached.savedAt?'Önbellek yüklendi · '+new Date(cached.savedAt).toLocaleString('tr-TR'):'Önbellek yüklendi')}).catch(()=>undefined)},[])
 const saveBoreholesCache=async()=>{try{const saved=await window.api.fieldCache.saveBoreholes(boreholes);setStatusText('Sondaj önbelleğe alındı · '+new Date(saved.savedAt).toLocaleTimeString('tr-TR'))}catch(e){setStatusText('Sondaj önbellek kaydı başarısız: '+(e instanceof Error?e.message:String(e)))}} 
 const saveLabsCache=async()=>{try{const saved=await window.api.fieldCache.saveLaboratories(labs);setStatusText('Laboratuvar önbelleğe alındı · '+new Date(saved.savedAt).toLocaleTimeString('tr-TR'))}catch(e){setStatusText('Laboratuvar önbellek kaydı başarısız: '+(e instanceof Error?e.message:String(e)))}}
