...
function App(){
 ...
 useEffect(()=>{if(!project.id)updateProjectInfo({...project,id:crypto.randomUUID()})},[project.id])
 const saveBoreholesCache=async()=>{try{const saved=await window.api.fieldCache.saveBoreholes(project.id,boreholes);setStatusText('Sondaj önbelleğe alındı · '+new Date(saved.savedAt).toLocaleTimeString('tr-TR'))}catch(e){setStatusText('Sondaj önbellek kaydı başarısız: '+(e instanceof Error?e.message:String(e)))}} 
 const saveLabsCache=async()=>{try{const saved=await window.api.fieldCache.saveLaboratories(project.id,labs);setStatusText('Laboratuvar önbelleğe alındı · '+new Date(saved.savedAt).toLocaleTimeString('tr-TR'))}catch(e){setStatusText('Laboratuvar önbellek kaydı başarısız: '+(e instanceof Error?e.message:String(e)))}}
 ...
 const content:Record<ScreenId,ReactNode>={dashboard:<Dashboard onNavigate={setScreen}/>,'project-info':<ProjectInfoScreenV2/>,field:field,'borehole-log':<BoreholeLogScreen boreholes={boreholes} labs={labs} onBoreholesChange={setBoreholes}/>,profile:<IdealizedSoilProfileScreen boreholes={boreholes} labs={labs} profile={idealizedSoilProfile} onChange={setIdealizedSoilProfile}/>, 'bearing-capacity':<BearingCapacityScreen profile={idealizedSoilProfile}/>,settlement:<IdealizedSettlementScreen profile={idealizedSoilProfile}/>,liquefaction:<Liquefaction boreholes={boreholes} labs={labs}/>,foundation:foundation,'jet-grout':<JetGroutEngineeringScreen/>,report:<EngineeringReportScreen boreholes={boreholes} labs={labs} profile={idealizedSoilProfile}/>}
