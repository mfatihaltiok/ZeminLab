...
export function WorkspaceShell({screen,onScreenChange,onNewProject,onOpenProject,onSaveProject,children}:WorkspaceShellProps){
  ...
  return <div className="app-shell">
    <div className="title-bar">
      ...
      <div className="window-controls"><button aria-label="Küçült" onClick={()=>void window.api.window.minimize()}>_</button><button aria-label="Büyüt / geri al" onClick={()=>void window.api.window.maximizeToggle()}>□</button><button className="close" aria-label="ZeminLab'ı kapat" onClick={()=>void window.api.window.close()}>×</button></div>
    </div>
    <div className="menu-bar"><button onClick={onNewProject}>Dosya</button><button onClick={()=>go('project-info')}>Düzen</button><button onClick={()=>go('dashboard')}>Görünüm</button><button onClick={()=>go('profile')}>Tanımlar</button><button onClick={()=>go('bearing-capacity')}>Analiz</button><button onClick={()=>go('foundation')}>Tasarım</button><button onClick={()=>go('report')}>Rapor</button><button onClick={()=>go('dashboard')}>Yardım</button></div>
    <div className="toolbar">
      ...
      <div className="toolbar-divider"/><div className="toolbar-section"><button className="tool-button" onClick={()=>go('field')}><span>⌕</span><b>Sondaj</b></button><button className="tool-button" onClick={()=>go('field')}><span>N</span><b>SPT</b></button><button className="tool-button" onClick={()=>go('field')}><span>▥</span><b>Laboratuvar</b></button></div>
      <div className="toolbar-divider"/><div className="toolbar-section"><button className="tool-button emphasis" onClick={()=>go('bearing-capacity')}><span>∑</span><b>Analiz</b></button><button className="tool-button emphasis" onClick={()=>go('foundation')}><span>⌂</span><b>Temel</b></button><button className="tool-button emphasis" onClick={()=>go('jet-grout')}><span>◉</span><b>Jet Grout</b></button><button className="tool-button emphasis" onClick={()=>go('report')}><span>▤</span><b>Rapor</b></button></div>
