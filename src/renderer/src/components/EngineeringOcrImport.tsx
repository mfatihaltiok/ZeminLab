...
const LAB_FIELDS:Array<[string,string[]]>=[
  ...
  ['density',['yogunluk','density']],['porosity',['porozite','porosity']],['voidRatio',['bosluk orani','void ratio','e']],
...
export function EngineeringOcrImport({mode,laboratoryTargets=[],onSptImport,onLaboratoryImport}:{mode:Mode;laboratoryTargets?:Array<{id:string;sampleId:string;depth:number}>;onSptImport?:(rows:SptCandidate[])=>void;onLaboratoryImport?:(targetId:string,rows:LabCandidate[])=>void}){
  ...
  const analyze=async(dataUrl:string)=>{setBusy(true);setMessage('Belge yapısı ve OCR analiz ediliyor…');setReview(false);setAnalysis(null);try{const result=await window.api.ocr.analyzeImage(dataUrl);if(!result.ok)throw new Error(result.error??'Belge analizi başarısız.');const found=result.lines?.length??0,tableCount=result.document?.tables?.length??0;const extracted=mode==='spt'?parseSpt(result.lines??[]).length:parseLaboratory(result.lines??[],result.document?.tables??[]).length;if(!found&&!result.document?.text?.trim())throw new Error('Belgede güvenilir metin bulunamadı.');setAnalysis(result);setReview(true);setMessage(found+' OCR satırı, '+tableCount+' tablo yapısı bulundu. '+extracted+' doğrulanmış '+(mode==='spt'?'SPT':'laboratuvar')+' veri adayı çıkarıldı. Sonuçlar otomatik kaydedilmez.')}catch(error){setMessage(error instanceof Error?error.message:'Belge analizi başarısız.')}finally{setBusy(false)}}
