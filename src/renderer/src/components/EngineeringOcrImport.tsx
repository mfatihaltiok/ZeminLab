...
const boxCenterY=(box:unknown)=>{if(!Array.isArray(box)||box.length<4)return null;const n=box.map(Number);if(n.slice(0,4).some(v=>!Number.isFinite(v)))return null;return (n[1]+n[3])/2}
const boxCenterX=(box:unknown)=>{if(!Array.isArray(box)||box.length<4)return null;const n=box.map(Number);if(n.slice(0,4).some(v=>!Number.isFinite(v)))return null;return (n[0]+n[2])/2}

function parseSpt(lines:OcrLine[]):SptCandidate[]{
  const direct=lines.flatMap(line=>{const nums=numberTokens(line.text);if(nums.length<3||nums.length>8)return[];const depth=nums[0];if(depth<0||depth>200)return[];const n=nums.slice(1,4).filter(v=>v>=0&&v<=999);if(n.length<2)return[];return[{depth,n1:n[0],n2:n[1],n3:n[2],score:line.score??0,raw:line.text}]})
  if(direct.length)return direct

  // PaddleOCR can return one table cell per OCR line. Rebuild rows from the
  // bounding-box Y positions, then read cells left-to-right.
  const cells=lines.map((line,index)=>({line,index,x:boxCenterX(line.box),y:boxCenterY(line.box),nums:numberTokens(line.text)})).filter(c=>c.x!==null&&c.y!==null&&c.nums.length===1)
  cells.sort((a,b)=>(a.y as number)-(b.y as number)||(a.x as number)-(b.x as number))
  const rows:{y:number;cells:typeof cells}[]=[]
  for(const cell of cells){const last=rows[rows.length-1];if(last&&Math.abs(last.y-(cell.y as number))<=Math.max(12,Math.abs(last.y)*0.015)){last.cells.push(cell);last.y=(last.y*(last.cells.length-1)+(cell.y as number))/last.cells.length}else rows.push({y:cell.y as number,cells:[cell]})}
  return rows.flatMap(row=>{
    const ordered=row.cells.sort((a,b)=>(a.x as number)-(b.x as number));const nums=ordered.map(c=>c.nums[0]);
    if(nums.length<3)return[];const depth=nums[0];if(depth<0||depth>200)return[];const n=nums.slice(1,4).filter(v=>v>=0&&v<=999);if(n.length<2)return[];
    const score=ordered.reduce((sum,c)=>sum+(c.line.score??0),0)/ordered.length;return[{depth,n1:n[0],n2:n[1],n3:n[2],score,raw:ordered.map(c=>c.line.text).join(' | ')}]
  })
}
...
export function EngineeringOcrImport({mode,laboratoryTargets=[],onSptImport,onLaboratoryImport}:{mode:Mode;laboratoryTargets?:Array<{id:string;sampleId:string;depth:number}>;onSptImport?:(rows:SptCandidate[])=>void;onLaboratoryImport?:(targetId:string,rows:LabCandidate[])=>void}){
  ...
  useEffect(()=>{const updateHost=()=>{const nextHost=document.querySelector<HTMLElement>('.field-content');const active=document.querySelector<HTMLElement>('.field-tabs button.active')?.textContent?.trim();setHost(nextHost);setVisible(mode==='spt'?active==='SPT':active==='Laboratuvar')};updateHost();const observer=new MutationObserver(updateHost);observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});return()=>observer.disconnect()},[mode])
  useEffect(()=>{if(!targetId&&laboratoryTargets[0])setTargetId(laboratoryTargets[0].id)},[laboratoryTargets,targetId])
  const analyze=async(dataUrl:string)=>{setBusy(true);setMessage('PaddleOCR görüntüyü okuyor…');setReview(false);try{const result=await window.api.ocr.analyzeImage(dataUrl);if(!result.ok)throw new Error(result.error??'OCR başarısız.');const next=result.lines??[];setLines(next);setReview(true);setMessage(next.length?`${next.length} OCR satırı bulundu. Tablo adayları aşağıda.`:'OCR metin bulamadı. Görsel çözünürlüğünü veya kırpmasını kontrol edin.')}catch(error){setMessage(error instanceof Error?error.message:'OCR başarısız.')}finally{setBusy(false)}}
  ...
  const panel=<section className="engineering-note engineering-ocr-panel" style={{marginBottom:12}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap'}}><div><strong>{mode==='spt'?'SPT VERİLERİNİ GÖRÜNTÜDEN OKU':'LABORATUVAR VERİLERİNİ GÖRÜNTÜDEN OKU'}</strong><div style={{fontSize:12,opacity:.75}}>PNG/JPG seçebilir veya <b>Ctrl+V</b> ile panodaki görseli yapıştırabilirsiniz. OCR sonucu doğrudan kaydedilmez.</div></div><div style={{display:'flex',gap:8,alignItems:'center'}}>{mode==='laboratory'&&<select value={targetId} onChange={e=>setTargetId(e.target.value)} disabled={!laboratoryTargets.length}><option value="">Hedef numune seçin</option>{laboratoryTargets.map(t=><option key={t.id} value={t.id}>{t.sampleId} · {t.depth.toFixed(2)} m</option>)}</select>}<button className="command-button" disabled={busy} onClick={()=>inputRef.current?.click()}>{busy?'Okunuyor…':'Görsel seç'}</button><input ref={inputRef} hidden type="file" accept="image/png,image/jpeg" onChange={e=>void onFile(e.target.files?.[0])}/></div></div>{message&&<div style={{marginTop:8}}>{message}</div>}{review&&<div style={{marginTop:12,overflowX:'auto'}}><table className="engineering-grid"><thead><tr>{mode==='spt'?<><th>Derinlik</th><th>n1</th><th>n2</th><th>n3</th><th>OCR</th></>:<><th>Alan</th><th>Değer</th><th>OCR</th></>}</tr></thead><tbody>{mode==='spt'?(candidates as SptCandidate[]).map((r,i)=><tr key={i}><td>{r.depth}</td><td>{r.n1??'—'}</td><td>{r.n2??'—'}</td><td>{r.n3??'—'}</td><td>{r.score?`${(r.score*100).toFixed(0)}%`:r.raw}</td></tr>):(candidates as LabCandidate[]).map((r,i)=><tr key={i}><td>{r.field}</td><td>{r.value}</td><td>{r.score?`${(r.score*100).toFixed(0)}%`:r.raw}</td></tr>)}</tbody></table><div style={{display:'flex',justifyContent:'flex-end',marginTop:8}}><button className="command-button primary" disabled={!candidates.length||(mode==='laboratory'&&!targetId)} onClick={importRows}>Kontrol ettim, verileri içeri aktar</button></div></div>}</section>
