...
export default function FieldInvestigation({ boreholes, labs, onBoreholesChange, onLabsChange }: Props) {
  const [selectedId,setSelectedId]=useState(boreholes[0]?.id??''); const [tab,setTab]=useState<'spt'|'lab'|'log'>('spt')
  ...
  if(!active) return <div className="field-empty"><b>Sondaj verisi yok.</b><button onClick={()=>{const b=createEmptyBorehole(boreholes.length+1);onBoreholesChange([b]);setSelectedId(b.id)}}>+ İlk Sondajı Oluştur</button></div>
  const changeBorehole=(next:BoreholeRecord)=>onBoreholesChange(boreholes.map(b=>b.id===next.id?next:b)); const addBorehole=()=>{const b=createEmptyBorehole(boreholes.length+1);onBoreholesChange([...boreholes,b]);setSelectedId(b.id)}
