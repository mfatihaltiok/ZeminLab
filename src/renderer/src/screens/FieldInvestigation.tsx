...
function syncLabs(labs: LaboratoryRecord[], borehole: BoreholeRecord): LaboratoryRecord[] {
  ...
  const next = borehole.spt.filter(spt => spt.laboratoryLinked !== false).map((spt, i) => {
    ...
    return { id, boreholeId: borehole.id, sampleId: old?.sampleId ?? `${spt.testType}-${String(i + 1).padStart(2, '0')}`, depth: spt.depth, sampleType: spt.testType, soilCode: spt.soilCode, soilDescription: spt.soilDescription,
      ...
      elasticModulus: old?.elasticModulus, poissonRatio: old?.poissonRatio, hydrometer075: old?.hydrometer075, hydrometer002: old?.hydrometer002, directShearC: old?.directShearC, directShearPhi: old?.directShearPhi,
      density: old?.density, porosity: old?.porosity, voidRatio: old?.voidRatio, c: old?.c, phi: old?.phi, finesContent: old?.finesContent, source: old?.source ?? spt.source, confirmed: old?.confirmed ?? false, notes: old?.notes } satisfies LaboratoryRecord
  })
...
function SptAnalysis({ borehole, labs }: { borehole: BoreholeRecord; labs: LaboratoryRecord[] }) {
  ...
  return <div className="engineering-grid-wrap"><div className="grid-toolbar"><b>SPT HESAP İZİ</b><span>N30 · N60 · σv · σ′v · CN · (N1)60</span><span className="spt-correction-note">CN = min(1.70, 9.78 / √σ′v) · σ′v profili eksiksiz değilse düzeltme uygulanmaz</span></div><table className="engineering-grid engineering-grid-analysis"><thead><tr><th>Derinlik</th><th>N30</th><th>N60</th><th>σv</th><th>σ′v</th><th>CN</th><th>(N1)60</th><th>Dilatasyon</th></tr></thead><tbody>{rows.map(({record,derived})=><tr key={record.id}><td>{fmt(record.depth)}–{fmt(experimentDepthTo(record))}</td><td>{fmt(derived.nField,0)}</td><td>{fmt(derived.n60)}</td><td>{fmt(derived.verticalStress)}</td><td>{fmt(derived.effectiveStress)}</td><td>{derived.effectiveStress!=null?fmt(derived.overburdenCorrection):'—'}</td><td>{derived.effectiveStress!=null?fmt(derived.n1_60):'—'}</td><td>{derived.dilatancyApplied?`Uygulandı → ${fmt(derived.n60DilatancyCorrected)}`:'—'}</td></tr>)}</tbody></table></div>
}
...
function SondajLog({ borehole, labs }: { borehole: BoreholeRecord; labs: LaboratoryRecord[] }) {
  const depth=Math.max(borehole.totalDepth,1); const rows=[...borehole.spt].sort((a,b)=>a.depth-b.depth)
  ...
  return <div className="log-only"><div className="log-summary"><div><span>Kuyu</span><b>{borehole.name}</b></div><div><span>Toplam Derinlik</span><b>{borehole.totalDepth>0?`${fmt(borehole.totalDepth,1)} m`:'—'}</b></div><div><span>YASS</span><b>{borehole.groundwaterDepth===undefined?'—':`${fmt(borehole.groundwaterDepth)} m`}</b></div><div><span>Deney</span><b>{rows.length} / Lab {labCount}</b></div></div><div className="log-graphic"><div className="log-ruler">{Array.from({length:Math.floor(depth)+1},(_,i)=>i).filter(d=>d%2===0||d===depth).map(d=><span key={d} style={{top:`${d/depth*100}%`}}>{d} m</span>)}</div><div className="log-column">{segments.length===0&&<div className="log-empty">SPT verisi girildiğinde zemin profili burada otomatik oluşturulur.</div>}{segments.map(({row,from,to,soil})=><div key={row.id} className={`log-layer ${soil?.colorClass??'fill'}`} style={{top:`${from/depth*100}%`,height:`${Math.max(1,(to-from)/depth*100)}%`}}><b>{row.soilCode||'—'}</b><span>{row.soilDescription||soil?.description||'Zemin sınıfı seçilmedi'}</span></div>)}{borehole.groundwaterDepth!==undefined&&borehole.groundwaterDepth<=depth&&<div className="log-water" style={{top:`${borehole.groundwaterDepth/depth*100}%`}}><span>YASS {fmt(borehole.groundwaterDepth)} m</span></div>}</div></div><div className="log-note">Bu ekran görsel sondaj logudur. Ayrıntılı kullanıcı müdahalesi SPT ve Laboratuvar tablolarından yapılır.</div></div>
}
