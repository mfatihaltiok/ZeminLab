...
export function JetGroutEngineeringScreen() {
  ...
  const [soilEs,setSoilEs]=useState('5000'), [columnEs,setColumnEs]=useState('100000'), [area,setArea]=useState('16'), [load,setLoad]=useState('800')
  ...
  const r=useMemo(()=>jetGroutEngineering({columnDiameter:Number(d),spacing:Number(spacing),layout,qSoil:Number(soil),qColumn:Number(column),EsSoil:Number(soilEs),EsColumn:Number(columnEs),load:Number(load),foundationArea:Number(area),foundationThickness:Number(thickness),columnFrictionAngle:Number(phi),cohesion:Number(c),frictionAngle:Number(angle),verticalLoad:Number(load),horizontalLoad:Number(hload)}),[d,spacing,layout,soil,column,soilEs,columnEs,load,area,thickness,phi,c,angle,hload])
  return <Frame screen="jet-grout">
    <Source>TBDY 2018 Ek 16D.3–16D.4: rijit elemanlarla iyileştirme ve hedeflenen iyileşme derecesinin tanımlanması. Kompozit birim-hücre hesabı ZeminLab motorudur. Priebe bölümü yalnızca taş kolon/vibro-replacement referans taraması olarak gösterilir, Jet Grout hesabına uygulanmaz.</Source>
    <Card title="JET GROUT · İLERİ TASARIM">
      <div className="form-grid">
        ...
        <Field label="Zemin qult" value={soil} onChange={setSoil}/><Field label="Kolon qult" value={column} onChange={setColumn}/>
    ...
    <div className="metric-strip">
      <Metric label="Alan oranı ar" value={(r.areaReplacementRatio*100).toFixed(2)} unit="%"/>
      <Metric label="Kompozit kapasite" value={r.compositeCapacity.toFixed(2)} unit="kPa" tone="primary"/>
      ...
      <Metric label="β stress concentration" value={r.stressConcentrationFactor.toFixed(2)}/>
    </div>
    <Card title="BİRİM HÜCRE · YÜK PAYLAŞIMI"><CalculationTrace title="Kompozit hesap zinciri" source="ZeminLab Jet Grout birim hücre motoru" rows={[
