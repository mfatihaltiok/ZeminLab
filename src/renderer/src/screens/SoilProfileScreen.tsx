...
function labForLayer(layer: LithologyLayer, labs: LaboratoryRecord[], boreholeId: string) {
  return labs
    .filter((lab) => lab.boreholeId === boreholeId && lab.depth >= layer.from && lab.depth <= layer.to)
    .sort((a, b) => Math.abs(a.depth - (layer.from + layer.to) / 2) - Math.abs(b.depth - (layer.from + layer.to) / 2))[0]
}
  ...
  return borehole.lithology
    ...
    .map((layer) => {
      ...
      return {
        ...
        gamma: layer.unitWeight ?? lab?.unitWeight ?? 0,
        gammaSat: layer.saturatedUnitWeight ?? 0,
        cohesion: layer.cohesion ?? lab?.c ?? 0,
        phi: layer.frictionAngle ?? lab?.phi ?? 0,
        fines: layer.finesContent ?? lab?.finesContent ?? 0,
        gammaSource,
        gammaSatSource,
        cSource,
        phiSource,
        finesSource,
        labSample: lab?.sampleId
      }
    })
}
...
export function SoilProfileScreen({ boreholes, labs }: { boreholes: BoreholeRecord[]; labs: LaboratoryRecord[] }) {
  ...
  const stressRows = layers.map((layer) => {
    ...
    return [
      layer.soil,
      `${layer.top.toFixed(2)}–${layer.bottom.toFixed(2)}`,
      formatValue(layer.gamma),
      formatValue(layer.cohesion),
      formatValue(layer.phi),
      result ? result.sigmaV.toFixed(1) : '—',
      result ? result.sigmaVPrime.toFixed(1) : '—'
    ]
  })
  ...
  return <Frame screen="profile">
    <Source>{SOURCE_NOTES.investigation} Profil artık seçilen sondajın litolojisini ve aynı derinlik aralığındaki laboratuvar numunelerini kullanır. Eksik mühendislik parametreleri varsayılan değerle doldurulmaz. TBDY 2018'e göre deprem tasarımında yerel zemin sınıfı için gerekli veriler ayrıca doğrulanmalıdır. citeturn0search12</Source>
    ...
    <Card title="KATMAN / GERİLME TABLOSU">
      ...
      {missingCount > 0 && <Source>Gerilme hesabı için gerekli γ veya YAS altında γsat verisi eksik olan katmanlar “—” gösterilir. Uygulama mühendislik parametresi uydurmaz.</Source>}
    </Card>
    <Card title="PARAMETRE KAYNAĞI">
      <Table
        headers={['Katman', 'γ', 'γsat', 'c', 'φ', 'İnce dane', 'Laboratuvar']}
        rows={layers.map((layer) => [layer.soil, layer.gammaSource, layer.gammaSatSource, layer.cSource, layer.phiSource, layer.finesSource, layer.labSample ?? '—'])}
      />
    </Card>
  </Frame>
}
