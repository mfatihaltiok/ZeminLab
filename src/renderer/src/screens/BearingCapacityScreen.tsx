...
export function BearingCapacityScreen({profile}:{profile?:IdealizedSoilProfile}){
 ...
 const verticalLoad=forceToBase(Number(f.verticalLoad),p.unitSystem)
 const horizontalLoad=forceToBase(Number(f.horizontalLoad),p.unitSystem)
 const momentX=momentToBase(Number(f.momentX),p.unitSystem)
 const momentY=momentToBase(Number(f.momentY),p.unitSystem)
 ...
 return <Frame screen="bearing-capacity"><Source>{SOURCE_NOTES.bearing} Dört klasik literatür yöntemi ayrı, TBDY 2018 kontrolü ayrı gösterilir. TBDY yüzeysel temel dayanım katsayısı Tablo 16.2'ye göre γRv=1.40'tır.</Source>
  <Card title="PROJE TEMELİ VE AKTİF LİTERATÜR YÖNTEMİ"><div className="form-grid"><Metric label="Temel tipi" value={f.foundationType==='surekli'?'Sürekli (şerit)':f.foundationType==='radye'?'Radye':'Tekil'}/><label>Hesap yöntemi<select value={method} onChange={e=>setMethod(e.target.value as BearingMethod)}>{(['Terzaghi','Meyerhof','Hansen','Vesic'] as BearingMethod[]).map(x=><option key={x}>{x}</option>)}</select></label><Metric label="İzin verilen gross qallow" value={stress(generic.value.allowableGross).toFixed(2)} unit={units.stress} tone="primary"/><Metric label="Aktif qult" value={stress(generic.value.ultimate).toFixed(2)} unit={units.stress}/><Metric label="FS" value={FS.toFixed(2)}/></div></Card>
