...
export function BearingCapacityScreen() {
  ...
  return <Frame screen="bearing-capacity">
    ...
    <Card title="YÖNTEMLERİN YAN YANA HESAPLANMASI">
      <Table headers={['Yöntem','İzin verilen gross taşıma gücü']} rows={genericRows.map(r=>[r[0],`${stress(Number(r[1])).toFixed(2)} ${PROJECT_UNIT_LABELS.stress}`])}/>
      <CalculationTrace title={`${method} hesap zinciri`} source={generic.source} rows={generic.steps}/>
    </Card>
