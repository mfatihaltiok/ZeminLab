...
export function IdealizedSoilProfileScreen({boreholes,labs,profile,onChange}:Props){
 ...
 return <Frame screen="profile">
  ...
  {selected&&<div className="dashboard-grid"><Card title={'SEÇİLİ KATMAN · '+selected.order}><div className="profile-selected-properties"><div><b>Derinlik</b><span>{selected.topDepth.toFixed(2)} – {selected.bottomDepth.toFixed(2)} m</span></div><div><b>Kalınlık</b><span>{(selected.thickness||0).toFixed(2)} m</span></div><div><b>SPT</b><span>{selected.sourceSptRecordId?'Bağlı':'Seçilmedi'}</span></div><div><b>LAB</b><span>{selected.sourceLaboratoryRecordId?'Bağlı':'Seçilmedi'}</span></div><div><b>γ</b><span>{n(selected.gamma)}</span></div><div><b>c</b><span>{n(selected.cohesion)}</span></div><div><b>φ</b><span>{n(selected.frictionAngle)}°</span></div><div><b>Cc</b><span>{n(selected.compressionIndexCc)}</span></div><div><b>e₀</b><span>{n(selected.initialVoidRatio)}</span></div><div><b>E</b><span>{n(selected.elasticModulus)}</span></div></div></Card><Card title="KAYNAK"><div className="engineering-note">SPT zemin tanımı kaynaktır. Mühendislik parametreleri seçilen laboratuvar numunesinden taşınır.</div></Card></div>}
 </Frame>
}
