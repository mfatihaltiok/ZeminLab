...
export function IdealizedSoilProfileScreen({boreholes,labs,profile,onChange}:Props){
 ...
 const layers=p.layers,selected=layers[sel],maxDepth=Math.max(1,...layers.map(x=>x.bottomDepth),Number(project.foundationParameters.footingDepth)||0)
 const save=(next:IdealizedSoilLayer[])=>onChange({...p,layers:reflow(next),status:'TASLAK',frozenAt:undefined})
 const add=()=>{if(!spts.length)return;const used=new Set(layers.map(x=>x.sourceSptRecordId));const src=spts.find(x=>!used.has(x.spt.id))??spts[0];const next=[...layers,makeLayer(src,labs,layers.length+1)];setSel(next.length-1);save(next)}
 const remove=()=>{if(!selected)return;const next=layers.filter((_,i)=>i!==sel);setSel(Math.max(0,Math.min(sel,next.length-1)));save(next)}
 const move=(dir:number)=>{const j=sel+dir;if(!selected||j<0||j>=layers.length)return;const next=[...layers];[next[sel],next[j]]=[next[j],next[sel]];setSel(j);save(next)}
 const chooseSpt=(i:number,value:string)=>{const src=spts.find(x=>x.borehole.id+'::'+x.spt.id===value);if(!src)return;const lab=linkedLab(labs,src.borehole.id,src.spt.id,src.spt.depth),patch=labPatch(lab);save(layers.map((x,k)=>k!==i?x:{...x,soilName:src.spt.soilDescription??'Zemin tanımı seçilmedi',soilCode:src.spt.soilCode??'',boreholeIds:[src.borehole.id],sptRecordIds:[src.spt.id],laboratoryRecordIds:lab?[lab.id]:[],sourceBoreholeId:src.borehole.id,sourceSptRecordId:src.spt.id,sourceLaboratoryRecordId:lab?.id,representativeSptN:src.spt.n2!=null&&src.spt.n3!=null?src.spt.n2+src.spt.n3:undefined,parameterSources:lab?{...Object.fromEntries(Object.keys(patch).filter(k=>(patch as Record<string,unknown>)[k]!=null).map(k=>[k,{type:'LABORATUVAR',sampleIds:[lab.id]}]))}:{},...patch}))}
 const chooseLab=(i:number,labId:string)=>{const lab=labId?labs.find(x=>x.id===labId):undefined;save(layers.map((x,k)=>k!==i?x:{...x,sourceLaboratoryRecordId:lab?.id,laboratoryRecordIds:lab?[lab.id]:[],parameterSources:lab?{...x.parameterSources,...Object.fromEntries(Object.keys(labPatch(lab)).filter(key=>(labPatch(lab) as Record<string,unknown>)[key]!=null).map(key=>[key,{type:'LABORATUVAR',sampleIds:[lab.id]}]))}:{},...labPatch(lab)}))}
 const thickness=(i:number,v:string)=>save(layers.map((x,k)=>k===i?{...x,thickness:Math.max(0,Number(v)||0)}:x))
 ...
 return <Frame screen="profile">
  ...
  {selected&&<div className="dashboard-grid"><Card title={'SEÇİLİ KATMAN · '+selected.order}><div className="profile-selected-properties"><div><b>Derinlik</b><span>{selected.topDepth.toFixed(2)} – {selected.bottomDepth.toFixed(2)} m</span></div><div><b>Kalınlık</b><span>{(selected.thickness||0).toFixed(2)} m</span></div><div><b>SPT</b><span>{selected.sourceSptRecordId?'Bağlı':'Seçilmedi'}</span></div><div><b>LAB</b><span>{selected.sourceLaboratoryRecordId?'Bağlı':'Seçilmedi'}</span></div><div><b>γ</b><span>{n(selected.gamma)}</span></div><div><b>c</b><span>{n(selected.cohesion)}</span></div><div><b>φ</b><span>{n(selected.frictionAngle)}°</span></div><div><b>Cc</b><span>{n(selected.compressionIndexCc)}</span></div><div><b>e₀</b><span>{n(selected.initialVoidRatio)}</span></div><div><b>E</b><span>{n(selected.oedometricModulus)}</span></div></div></Card><Card title="KAYNAK"><div className="engineering-note">SPT zemin tanımı kaynaktır. Mühendislik parametreleri seçilen laboratuvar numunesinden taşınır.</div></Card></div>}
 </Frame>
}
