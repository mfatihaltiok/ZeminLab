...
export function IdealizedSoilProfileScreen({ boreholes, labs, profile, onChange }: { boreholes: BoreholeRecord[]; labs: LaboratoryRecord[]; profile?: IdealizedSoilProfile; onChange: (p: IdealizedSoilProfile) => void }) {
  ...
  return <Frame screen="profile">
    ...
    <div className="idealized-layout">
      <Card title="İDEALİZE KATMANLAR"><table className="data-table"><thead><tr>{['#','Üst','Alt','Kalınlık','Zemin','N-SPT','γ','c','φ'].map(x=><th key={x}>{x}</th>)}</tr></thead><tbody>{p.layers.map((x,i)=><tr key={x.id} onClick={()=>setSel(i)} className={i===sel?'idealized-selected':''}><td>{i+1}</td><td>{x.topDepth.toFixed(2)}</td><td>{x.bottomDepth.toFixed(2)}</td><td>{(x.bottomDepth-x.topDepth).toFixed(2)}</td><td>{x.soilName}</td><td>{x.representativeSptN?.toFixed(1)??'—'}</td><td>{x.gamma?.toFixed(2)??'—'}</td><td>{x.cohesion?.toFixed(2)??'—'}</td><td>{x.frictionAngle?.toFixed(1)??'—'}</td></tr>)}</tbody></table></Card>
      <aside className="idealized-properties"><div className="calculation-card-title">ÖZELLİKLER · {l?`KATMAN ${l.order}`:'—'}</div>{l&&<div className="idealized-form">
        ...
        <label>Sınıflandırma<input value={l.soilCode} disabled={p.status==='SABİTLENDİ'} onChange={e=>edit({soilCode:e.target.value})}/></label>
        <label>Üst (m)<input type="number" value={l.topDepth} disabled={p.status==='SABİTLENDİ'} onChange={e=>edit({topDepth:Number(e.target.value)})}/></label>
        <label>Alt (m)<input type="number" value={l.bottomDepth} disabled={p.status==='SABİTLENDİ'} onChange={e=>edit({bottomDepth:Number(e.target.value)})}/></label>
        <label>γ<input type="number" value={l.gamma??''} disabled={p.status==='SABİTLENDİ'} onChange={e=>edit({gamma:Number(e.target.value)})}/></label>
        <label>γsat<input type="number" value={l.gammaSat??''} disabled={p.status==='SABİTLENDİ'} onChange={e=>edit({gammaSat:Number(e.target.value)})}/></label>
        <label>c<input type="number" value={l.cohesion??''} disabled={p.status==='SABİTLENDİ'} onChange={e=>edit({cohesion:Number(e.target.value)})}/></label>
        <label>φ (°)<input type="number" value={l.frictionAngle??''} disabled={p.status==='SABİTLENDİ'} onChange={e=>edit({frictionAngle:Number(e.target.value)})}/></label>
        <label>CC<input type="number" value={l.compressionIndexCc??''} disabled={p.status==='SABİTLENDİ'} onChange={e=>edit({compressionIndexCc:Number(e.target.value)})}/></label>
        <label>CR<input type="number" value={l.recompressionIndexCr??''} disabled={p.status==='SABİTLENDİ'} onChange={e=>edit({recompressionIndexCr:Number(e.target.value)})}/></label>
